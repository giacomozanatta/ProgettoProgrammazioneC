...
int inserisci_una_bomba(matrice* campo, int righe, int colonne, int rig, int col){
	...
	aggiorna_post_inserimento_bomba(campo, righe, colonne, rig, col); /*aggiorno celle limitrofe alla bomba. in pratica aggiunge +1 al valore delle celle limitrofe.*/
	printf("OK!");
...
int crea_campo_vuoto(matrice* campo, int righe, int colonne){
	...
	*campo = (matrice) malloc(sizeof(cella*)*righe); 
	if(! (*campo))
		return 1;
	for(i=0; i<righe; i++){
		(*campo)[i] = (cella*) malloc(sizeof(cella)*colonne); /*allocco il campo in memoria dinamica*/
		if(! ((*campo)[i])){ /*se si è verificato un errrore, libero la memoria dinamica allocata precedentemente*/
			for(j=0; j<i; j++)
				free((*campo)[j]);
			free(*campo);
			*campo=NULL;
	...
	printf("\tOK!. ");
	inizializza_campo(campo, righe, colonne); /*inizializzo il campo appena creato*/
...
void scopri_cella_aux(matrice* campo, int righe, int colonne, int rig, int col, int* celle_scoperte){
	...
	if((*campo)[rig][col].marcata == 0){ /*se non è marcata...*/
		if((*campo)[rig][col].scoperta == 0 ){ /*e se non è stata scoperta...*/
			(*campo)[rig][col].scoperta++; /*incremento scoperta*/
			...
			if((*campo)[rig][col].value==0){ /*se è nulla, allora */
				for(i = rig-1; i<=rig+1; i++)
					for(j = col-1; j<=col+1; j++){
						/*controllo se sono posizioni corrette [DA OTTIMIZZARE]*/
						if(i>=0 && i<righe && j>=0 && j<colonne)
							scopri_cella_aux(campo, righe, colonne, i, j, celle_scoperte); /*scopro la cella limitrofa*/
				}
			}
		}
		else if((*campo)[rig][col].value>0)
			(*campo)[rig][col].scoperta++;
		/*se è già stata scoperta, la 'riscopro'. questa tecnica permette di annullare le mosse */
...
void annulla_mossa(matrice* campo, int righe, int colonne, int rig, int col, int* celle_scoperte){
	...
	if((*campo)[rig][col].marcata == 0){ /*se non è marcata...*/
		if((*campo)[rig][col].scoperta == 1){
			(*campo)[rig][col].scoperta--;
			/*se non è stata scoperta*/
			(*celle_scoperte)--; /*decremento cella scoperta*/
			if((*campo)[rig][col].value==0){ /*se è nulla, allora */
				for(i = rig-1; i<=rig+1; i++)
					for(j = col-1; j<=col+1; j++){
						/*controllo se sono posizioni corrette [DA OTTIMIZZARE]*/
			...
			(*campo)[rig][col].scoperta--; /*decremento scoperta. se è pari a 0 dopo il decremento, viene diminuito celle scoperte*/
			if((*campo)[rig][col].scoperta==0)
				(*celle_scoperte)--;
		}
...
/**marca cella: marca o smarca una cella
	valore di ritorno: 0 se posso marcarla/smarcarla
						1 se la cella è già stata scoperta (quindi non posso marcarla)
*/
