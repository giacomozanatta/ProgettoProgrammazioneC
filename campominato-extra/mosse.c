...
int inizializza_mosse(mosse* moves, int max_mosse){
    ...
    moves->dim_mosse = (max_mosse*(max_mosse-1)/2)+1;
    ...
    if(!moves->elementi)
        ...
        while(moves->elementi && annullamenti){
            if(pop(moves->buff_mosse, moves->dim_mosse, &(moves->idx_coda), &(moves->elementi), &riga, &colonna)==0)
                annulla_mossa(campo, righe, colonne, riga, colonna, celle_coperte);
            annullamenti--;
        }
...
int posso_annullare(mosse moves){
    return (moves.mosse_da_annullare>moves.max_mosse) ? 0 : 1;
}
